<h1 align="center">👋 Hey there, I'm <span style="color:#00BFFF">René </span></h1>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=Software+Engineer+%26+Data+Scientist;ML+Systems+for+Enterprise;AI+%C2%B7+Systems+Engineering+%C2%B7+Finance;Building+in+Rust+%26+Kubernetes&font=Fira+Code&center=true&width=500&height=50&duration=2500&pause=1200&color=00C6FF&vCenter=true&size=22" alt="Typing SVG" />
</p>



<p align="center">
  <img src="https://img.shields.io/badge/SAP%20Certified-S%2F4HANA%20Cloud-blue?style=flat-square&logo=sap" alt="SAP Certified" />
  &nbsp;
  <img src="https://img.shields.io/badge/Location-Germany-000000?style=flat-square&logo=google-maps" alt="Location" />
</p>

---

## 💫 About Me 
- 🧑‍💻 **Software Engineer & Data Scientist**  based in Germany.
- ⚙️ Focused on **ML Systems for enterprise contexts**—bridging the gap between AI engineering, system engineering, and robust infrastructure.
- 🦀 Currently expanding my expertise in **Rust**, **Linux**, and potentially **Kubernetes** in the future.
- 📝 I use this frontpage as a personal notekeeping space to track my projects, ideas, and reading log.

## 🧠 Languages & Tools

<p align="center">
  <img src="https://github.com/devicons/devicon/raw/master/icons/rust/rust-original.svg" width="45" title="Rust"/>
  <img src="https://github.com/devicons/devicon/raw/master/icons/python/python-original.svg" width="45" title="Python"/>
  <img src="https://github.com/devicons/devicon/raw/master/icons/linux/linux-original.svg" width="45" title="Linux"/>
  <img src="https://github.com/devicons/devicon/raw/master/icons/docker/docker-original.svg" width="45" title="Docker"/>
  <img src="https://github.com/devicons/devicon/raw/master/icons/kubernetes/kubernetes-plain.svg" width="45" title="Kubernetes"/>
  <img src="https://github.com/devicons/devicon/raw/master/icons/neovim/neovim-original.svg" width="45" title="Neovim"/>
  <img src="https://github.com/devicons/devicon/raw/master/icons/git/git-original.svg" width="45" title="Git"/>
  <img src="https://cdn.simpleicons.org/sap/008fd3" width="45" title="SAP"/>
</p>

---

## 🏗️ Portfolio Projects

1. 🌉 **[sap-to-ai-bridge]** — SAP table data to semantic search via RAG.<br/>![Progress](https://geps.dev/progress/0)
2. 🦀 **[rust-llm-experiment-tracker]** — CLI in Rust, published cargo crate.<br/>![Progress](https://geps.dev/progress/0)
3. 🧠 **[attention-tutorial-rust]** — From-scratch attention in Rust, step-by-step with visualisations.<br/>![Progress](https://geps.dev/progress/0)
4. ☸️ **[llm-on-kubernetes]** — Deploying a 7B model on K8s with real cost/latency numbers.<br/>![Progress](https://geps.dev/progress/0)
5. 🌐 **[ml-in-browser-rust]** — Running a small LLM in the browser via Rust → WASM.<br/>![Progress](https://geps.dev/progress/0)
6. 📊 **[llm-eval-enterprise]** — Eval framework for enterprise-shaped tasks.<br/>![Progress](https://geps.dev/progress/0)

*(Note: Detailed READMEs and code for each project are available in their respective repositories).*

---

## 📚 Reading Log
A public archive of books I have read to deepen my understanding of ML and Systems:

**🤖 Machine Learning & AI**
* *Deep Learning* (Goodfellow) — ![Progress](https://geps.dev/progress/100)
* *AI Engineering* (Huyen) — ![Progress](https://geps.dev/progress/50)
* *Agentic Design Patterns* (Gulli) — ![Progress](https://geps.dev/progress/0)
* *The AI Conundrum* (Briggs) — ![Progress](https://geps.dev/progress/100)
* *AI: A Systems Approach* (Martinez/Kifle) — ![Progress](https://geps.dev/progress/100)

**⚙️ Systems & Software Engineering**
* *Designing Data-Intensive Applications* (Kleppmann) — ![Progress](https://geps.dev/progress/100)
* *Pragmatic Programmer* (Hunt/Thomas) — ![Progress](https://geps.dev/progress/100)
* *Clean Code* (Martin) — ![Progress](https://geps.dev/progress/100)

**📈 Statistics, Economics & Finance**
* *Elements of Statistical Learning* (Hastie et al.) — ![Progress](https://geps.dev/progress/0)
* *Household Dynamics* (Lord) — ![Progress](https://geps.dev/progress/100)
* *Forecasting for Economics and Business* (González-Rivera) — ![Progress](https://geps.dev/progress/100)
* *Introductory Econometrics for Finance* (Brooks) — ![Progress](https://geps.dev/progress/100)
* *The Future of Money* (Prasad) — ![Progress](https://geps.dev/progress/100)

---

## 📝 Posts & Paper Notes
Topics and papers I have researched and posted about:
### Posts

### Papers
* 📄 *Attention Is All You Need* (Vaswani et al., NeurIPS 2017) — ![Progress](https://geps.dev/progress/100)
* 📈 *Scaling Laws for Neural Language Models* (Kaplan et al., 2020) — ![Progress](https://geps.dev/progress/100)
* 🚀 *The Power of 10 — NASA's Rules for Safety-Critical Code* (Holzmann, 2006) — ![Progress](https://geps.dev/progress/100)
* 💻 *JSF C++ Coding Standards* (Lockheed Martin, 2005) — ![Progress](https://geps.dev/progress/100)

---
<div align="center">
  <i>This profile is a strictly personal log of my coding projects and reading materials.</i>
</div>
